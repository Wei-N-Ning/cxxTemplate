...
struct RoundUpPow2
{
    enum
    {
        value = RoundUpPow2< N / 2, C * 2 >::value
    };
};
...
struct RoundUpPow2< 0, C >
{
    enum
    {
        value = ( C > ALIGNMENT_MAX ) ? ALIGNMENT_MAX : C
    };
};
...
struct Align
{
    enum
    {
        value = RoundUpPow2< sizeof( T ) * W - 1 >::value
    };
};
...
TEST_CASE( "expect `empty class` have non-zero size due to alignment" )
{
    using namespace std;
