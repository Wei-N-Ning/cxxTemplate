...
public:
    ...
    // definition:
    // see:
    // https://stackoverflow.com/questions/4039979/some-compiler-errors-concerning-an-overloaded-operator-on-a-template-in-c
    // A common pattern for friend free functions in templates is defining the function in
    // place:
    friend std::ostream& operator<<( std::ostream& os, const Stack< T >& st )
    {
        std::copy(
          st.vec.cbegin(), st.vec.cend(), std::ostream_iterator< T >{ os, " " } );
        return os;
    }
