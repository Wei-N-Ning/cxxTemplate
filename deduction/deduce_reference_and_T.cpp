...
//
// UPDATE
// auto and auto& (similarly const auto and const auto&) are completely two different
// things!! If I want to use value/object semantics, make sure I pay greater attention to
// this a getter that returns mutable reference is useless because heavy_obj is a copy not
// a reference auto heavy_obj = get_heavy_obj();
...
processRef( T& param )
{
    static_assert( ( std::is_same< T, Type_ >::value ) );
    static_assert( ( std::is_same< T&, ParamType_ >::value ) );
}

template< typename Type_, typename ParamType_, typename T >
void
processConstRef( const T& param )
{
    static_assert( ( std::is_same< T, Type_ >::value ) );
    static_assert( ( std::is_same< const T&, ParamType_ >::value ) );
}

template< typename Type_, typename ParamType_, typename T >
void
processPointer( T* param )
{
    static_assert( ( std::is_same< T, Type_ >::value ) );
    static_assert( ( std::is_same< T*, ParamType_ >::value ) );
}
...
verifyTAndParamType()
{
    ...
    processUniversalRef< int, int&& >(
      13 ); // 13 is a rvalue, ParamType_ is a rvalue-reference;
