...
public:
    using C =
      std::conditional_t< ( Size > 8 ), std::vector< Elem >, std::array< Elem, Size > >;
...
{
    using Type =
      typename std::conditional_t< std::is_integral_v< T > && !std::is_same_v< T, bool >,
                                   MakeUnsignedT< T >,
                                   IdentityT< T > >::Type; // ::Type is guaranteed to work
...
TEST_CASE( "test drive is_same_v" )
{
    static_assert( std::is_same_v< unsigned int, to_unsigned< int > > );

    static_assert( std::is_same_v< unsigned char, to_unsigned< char > > );

    static_assert( std::is_same_v< bool, to_unsigned< bool > > );
