...
TEST_CASE( "std::result_of_t<> example" )
{
    ...
    static_assert( std::is_same_v< decltype( result1 )::Type, int > );

    OpResult result2(
      []( const auto& lhs, const auto& rhs ) { return lhs > rhs ? lhs : rhs; }, 1, 'c' );
    static_assert( std::is_same_v< decltype( result2 )::Type, int > );
    ...
    OpResult result3(
      ...
      // but not mut ref
      []( auto&& lhs, auto&& rhs ) -> bool { return lhs > rhs; },
      1,
      2 );
    static_assert( std::is_same_v< decltype( result3 )::Type, bool > );
}
...
// conversions (covered in Standard Conversions) are performed to convert them to a common
// type.
TEST_CASE( "A/B comparison type (ternary operator return type)" )
{
    ...
    static_assert( std::is_same_v< long long, decltype( v3 )::value_type > );
}
...
TEST_CASE( "element type is of value" )
{
    ...
    static_assert( std::is_same_v< int, decltype( v3 )::value_type > );

    auto v4 = vecOp( v1, v2, []( int lhs, char rhs ) -> char { return lhs % 16 + rhs; } );
    static_assert( std::is_same_v< char, decltype( v4 )::value_type > );
}
...
TEST_CASE( "element type is of reference wrapper" )
{
    ...
    std::vector< std::reference_wrapper< int > > v1{ std::ref( v1orig[ 0 ] ),
                                                     std::ref( v1orig[ 1 ] ) };

    // to prove that v1 holds mut ref
    v1[ 1 ].get() = 20;

    std::vector< std::reference_wrapper< char > > v2{ std::ref( v2orig[ 0 ] ),
                                                      std::ref( v2orig[ 1 ] ) };
    ...
    auto v4 =
      vecOp( v1, v2, []( auto lhs, auto rhs ) -> char { return lhs % 16 + rhs; } );
    static_assert( std::is_same_v< char, decltype( v4 )::value_type > );
}
