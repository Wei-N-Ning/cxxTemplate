...
// C++17 in detail P/47
// the following variations of fold expressions with binary op exist
// (... op e)
// unary left fold: ((e1 op e2) op ...) op eN
// (init op ... op e)
// binary left fold: init op e1) op e2) op ...) op eN
// (e op ...)
// unary right fold: e1 op (... op (eN-1 op eN
// (e op ... op init)
// binary right fold: e1 op (... op (eN-1 op (eN op init
...
template< typename Map, typename Filter, typename Acc, typename T >
T
process( Map map, Filter filter, Acc acc, T init, T arg )
{
    T x = map( arg );
    if ( filter( x ) )
    {
