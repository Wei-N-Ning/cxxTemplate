...
int
bar_impl( const std::string& i_s )
{
    return 1;
}

int
bar_impl( int i, double d )
{
    return 2;
}
...
int
bar_impl( const std::string& i_s, std::string* o_str )
{
    *o_str = i_s;
    return 3;
}
...
TEST_CASE( "variadic function template" )
{
    foo( 1, 2.3, "asd", true );
    std::string name( "13" );
    CHECK_EQ( 1, bar( name ) );
...
template< typename... Ts >
std::ostream&
do_print( std::ostream& os, Ts... args )
{
    // L2638
    // use fold expression
...
void
recur_print( T head, const Ts&... tail )
{
    // L2611
